...
function generateItemSpecificsXml(category, listingData, categoryRequirements = null) {
  ...
  // Fixed import path - use correct path to your service
  let CategoryRequirementsService;
  let categoryService;
  
  try {
    // Try to import the service if available
    CategoryRequirementsService = require('../services/ebay/categoryRequirementsService');
    categoryService = new CategoryRequirementsService();
  } catch (error) {
    console.warn('CategoryRequirementsService not available, using fallback logic');
    categoryService = null;
  }

  try {
    // Process requirements-based specifics first if service is available
    if (categoryRequirements?.requiredAspects && categoryService) {
      ...
      console.warn('No category requirements provided or service unavailable, using fallback logic');
    }

    // Add category-specific fallback specifics - THIS IS THE KEY FIX
    addCategorySpecificFallbacksEnhanced(category, listingData, itemSpecifics, processedFields);
...
/**
 * ENHANCED category-specific fallback specifics - FIXES YOUR ISSUE
 */
function addCategorySpecificFallbacksEnhanced(category, listingData, itemSpecifics, processedFields) {
  // Import the category detector to help with detection
  const { categoryDetector } = require('./category-detector');
  
  // First, try to detect the actual category type
  let resolvedCategory = category?.toLowerCase() || '';
  
  // If the category doesn't directly indicate the type, use enhanced detection
  if (!['furniture', 'electronics', 'clothing', 'footwear'].some(cat => resolvedCategory.includes(cat))) {
    const detectionInput = `${category} ${listingData.title || ''} ${listingData.description || ''}`;
    const detectedCategory = categoryDetector.detectCategory(detectionInput);
    
    if (detectedCategory !== 'unknown') {
      resolvedCategory = detectedCategory;
      console.log(`Enhanced category detection: "${category}" -> "${detectedCategory}"`);
    }
  }

  // Now apply category-specific logic with enhanced detection
  if (categoryDetector.isFurnitureCategory(resolvedCategory) || 
      resolvedCategory.includes('chair') || 
      resolvedCategory.includes('table') ||
      resolvedCategory.includes('desk') ||
      resolvedCategory.includes('cantilever') ||
      resolvedCategory.includes('armchair')) {
    
    console.log('Applying furniture-specific item specifics');
    addFurnitureSpecifics(listingData, itemSpecifics, processedFields);
    
  } else if (categoryDetector.isFootwearCategory(resolvedCategory) || 
             resolvedCategory.includes('shoes') || 
             resolvedCategory.includes('boots')) {
    
    console.log('Applying footwear-specific item specifics');
    addFootwearSpecifics(listingData, itemSpecifics, processedFields);
    
  } else if (categoryDetector.isElectronicsCategory(resolvedCategory) || 
             resolvedCategory.includes('headphones') || 
             resolvedCategory.includes('audio')) {
    
    console.log('Applying electronics-specific item specifics');
    addElectronicsSpecifics(listingData, itemSpecifics, processedFields);
    
  } else if (categoryDetector.isClothingCategory(resolvedCategory) || 
             resolvedCategory.includes('apparel')) {
    
    console.log('Applying clothing-specific item specifics');
    addClothingSpecifics(listingData, itemSpecifics, processedFields);
  }
}
...
module.exports = {
  ...
  addCategorySpecificFallbacksEnhanced  // Export the enhanced function
};