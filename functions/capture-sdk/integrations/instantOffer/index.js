// functions/capture-sdk/integrations/instantOffer/index.js - COMMONJS VERSION
const { generateShippingLabel } = require('./shippingLabel.js');
const { calculateOffer } = require('./offerCalculator.js');

class InstantOfferIntegration {
  constructor(config) {
  ...
  async processPackageReceived(trackingNumber) {
    ...
    if (!offer) {
      throw new Error('Offer not found for tracking number');
    }

    // Process payout
    const payout = await this.processPayout(offer);
  ...
  async processPayout(offer) {
    switch (this.payoutProvider) {
      case 'stripe':
        return await this.stripePayout(offer);
      case 'paypal':
...
module.exports = { InstantOfferIntegration };