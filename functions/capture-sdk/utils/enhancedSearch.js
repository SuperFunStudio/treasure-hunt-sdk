// functions/capture-sdk/utils/enhancedSearch.js
// Universal Enhanced Search Query Builder
const { pickFirstDefined, ciGet, normalizeRecommendation } = require('./normalize.js');

/**
 * Build enhanced eBay search query from detailed item analysis
 * This is the main function that coordinates all the other helpers
 */
function buildEnhancedSearchQuery(itemData) {
  ...
  if (itemData.category && !excludeGeneric.some(term => itemData.category.toLowerCase().includes(term))) {
  ...
  if (itemData.materials && itemData.materials.length > 0) {
    ...
    if (primaryMaterial && !['unknown', 'generic'].includes(primaryMaterial.toLowerCase()) && !queryParts.some(part => part.toLowerCase().includes(primaryMaterial.toLowerCase()))) {
      queryParts.push(primaryMaterial);
    }
  }
  
  // 3. Add style if meaningful and distinctive
  if (itemData.style && !excludeGeneric.some(term => itemData.style.toLowerCase().includes(term)) && itemData.style.length > 3 && !queryParts.some(part => part.toLowerCase().includes(itemData.style.toLowerCase()))) {
  ...
  if (itemData.keyFeatures && itemData.keyFeatures.length > 0) {
    const bestFeature = itemData.keyFeatures.find(feature => 
      feature && feature.length > 3 && !['color', 'used', 'old', 'good', 'condition'].includes(feature.toLowerCase()) && !queryParts.some(part => part.toLowerCase().includes(feature.toLowerCase()))
    );
    if (bestFeature) {
  ...
  if (itemData.brand && !excludeGeneric.some(term => itemData.brand.toLowerCase().includes(term)) && itemData.brand.length > 2 && !queryParts.some(part => part.toLowerCase().includes(itemData.brand.toLowerCase()))) {
    queryParts.push(itemData.brand);
  }
  
  // 6. Add functional type if different from category
  if (itemData.functionalType && !queryParts.some(part => part.toLowerCase().includes(itemData.functionalType.toLowerCase()))) {
    const functionalWords = itemData.functionalType.split(' ');
    const newWords = functionalWords.filter(word => word.length > 3 && !queryParts.some(part => part.toLowerCase().includes(word.toLowerCase())));
 ...
 */
function addCategoryRefinements(query, itemData) {
  ...
  if (category.includes('shirt') || category.includes('jacket') || (category.includes('clothing') && !category.includes('military'))) {
    if (style.includes('vintage')) return query + ' vintage';
 ...
 */
function validateSearchQuery(query, itemData) {
  const validation = {
    isValid: true,
    issues: [],
    confidence: 'high'
...
module.exports = { buildEnhancedSearchQuery, addCategoryRefinements, validateSearchQuery };
