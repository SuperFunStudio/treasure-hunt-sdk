...
console.log('✅ Critical scan editor fixes loaded');
...
async function loadUserData() {
  try {
    ...
    if (doc.exists) {
      ...
      console.log('✅ Created new user document with Firebase Auth UID:', firebaseUID);
    }
  } catch (e) {
...
window.processAndPopulateForm = async function processAndPopulateForm(data) {
  console.log('🔄 Processing analysis data for form population:', data);
...
analyzeBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    console.log('✅ Created new scan record with ID:', currentScanId);
    ...
    showSuccess('✅ AI analysis complete! Form has been auto-populated with detected information.');
    goToStep('edit');
...
function generateAutoDescription(analysis) {
  ...
  if (analysis.keyFeatures?.length > 0) {
    parts.push('\nKey Features:');
    analysis.keyFeatures.forEach(f => parts.push(`• ${f}`));
  }
  if (analysis.condition?.issues?.length > 0) {
    parts.push('\nPlease note:');
    analysis.condition.issues.forEach(i => parts.push(`• ${i}`));
  }
...
function displayMarketAnalysis(marketAnalysis) {
  ...
  container.innerHTML = `
    <div style="background:#f8f9fa;border:1px solid #e9ecef;border-radius:8px;padding:16px;margin:16px 0;">
      <h4 style="margin:0 0 12px;color:#333;">💰 Market Analysis</h4>
...
function createEditForm(analysis) {
  console.log('📝 Creating edit form with analysis:', analysis);
...
function populateFormFromAnalysis(analysis, routes) {
  ...
  try {
      ...
      if (categorySelect) {
        ...
        if (isFallback) {
          ...
          if (!categoryWarning) {
            ...
            warningDiv.innerHTML = '⚠️ This category was auto-selected. Please verify it is correct.';
    ...
    console.log('✅ Form auto-population complete');
  } catch (e) {
    console.error('❌ Error auto-populating form:', e);
  }
}
...
function generateCategorySuggestions(detectedCategory) {
  ...
  return `
    <div class="smart-suggestions">
      <h4>💡 AI detected similar categories:</h4>
...
function generateBrandSuggestions(category) {
  ...
  return `
    <div class="smart-suggestions">
      <h4>💡 Popular brands in ${category}:</h4>
      <div class="suggestion-tags">
...
function validateAndCleanListingData(listingData) {
  console.log('🧹 Cleaning and validating listing data:', listingData);
  ...
  if (listingData.condition) {
    ...
    if (!allowedConditions.includes(listingData.condition)) {
      console.warn('⚠️ Invalid condition, defaulting to "good":', listingData.condition);
  ...
  if (listingData.title) {
    ...
    console.warn('⚠️ No title specified, generating default');
  ...
  if (listingData.description) {
    ...
    console.warn('⚠️ No description specified, generating default');
  ...
  if (listingData.category) {
    ...
    console.warn('⚠️ No category specified, defaulting to "electronics"');
  ...
  if (listingData.pricing) {
    ...
    console.warn('⚠️ No pricing specified, setting defaults');
    listingData.pricing = {
      buyItNowPrice: 9.99,
      startingPrice: 9.99,
      minimumOffer: 7.99
  ...
  if (!listingData.photos || !Array.isArray(listingData.photos)) {
    console.error('⚠️ No photos array found');
    listingData.photos = [];
  } else {
...
async function showListingOptions() {
  ...
  modal.innerHTML = `
    <div style="background: white; padding: 30px; border-radius: 15px; max-width: 500px; margin: 20px;">
      <h3 style="margin-bottom: 20px;">🚀 Ready to Create Your Listing?</h3>
      
      <div style="margin-bottom: 20px;">
        <strong>Item:</strong> ${editedData.brand} ${editedData.model}<br>
        <strong>Starting Price:</strong> ${editedData.price}<br>
        <strong>eBay Account:</strong> ✅ Connected as ${userData.ebay.sellerAccount}
      </div>
      ...
      <button onclick="closeModal()" style="position: absolute; top: 10px; right: 15px; background: none; border: none; font-size: 20px; cursor: pointer;">×</button>
    </div>
  `;
...
async function attemptEbayListing() {
  try {
    ...
    console.log('📋 Final listing data to send:', {
    ...
    if (result.success) {
      ...
      if (result.needsPolicies) {
        showSuccess(`🎯 eBay inventory created successfully! Your item (SKU: ${result.sku}) is ready to publish. Please set up your eBay seller policies first, then publish the offer.`);
        showPolicySetupInstructions(result);
      } else if (result.url) {
        showSuccess(`🎉 eBay listing created successfully! <a href="${result.url}" target="_blank" style="color: #2e7d32; text-decoration: underline;">View Listing</a>`);
        showListingSuccess(result);
      } else {
        showSuccess(`✅ eBay inventory and offer created! Your item is ready but needs to be published manually on eBay.`);
      }
    } else {
...
function showPolicySetupInstructions(result) {
  ...
  modal.innerHTML = `
    <div style="background: white; padding: 30px; border-radius: 15px; max-width: 600px; margin: 20px;">
      <h3 style="margin-bottom: 20px; color: #ff9800;">🏗️ Inventory Created - Policies Required</h3>
...
function showListingSuccess(result) {
  ...
  modal.innerHTML = `
    <div style="background: white; padding: 30px; border-radius: 15px; max-width: 500px; margin: 20px; text-align: center;">
      <h3 style="margin-bottom: 20px; color: #2e7d32;">🎉 Listing Created Successfully!</h3>
...
function showEbayConnectionPrompt() {
  ...
  prompt.innerHTML = `
    <h4 style="margin-bottom: 15px;">🛒 Connect eBay to Create Real Listings</h4>
...
function displayLoadedImages(images) {
  ...
  images.forEach((b64, idx) => {
    ...
    div.innerHTML = `<img src="data:image/jpeg;base64,${b64}" alt="Loaded image ${idx + 1}">
      <button class="remove-btn" data-index="${idx}">×</button><div class="info">Loaded</div>`;
  ...
  compressionStats.innerHTML = `<strong>✅ Images Loaded:</strong><br>${images.length} image${images.length > 1 ? 's' : ''} from previous scan`;
  compressionStats.style.display = 'block';
  analyzeBtn.disabled = false;
  analyzeBtn.textContent = '🔄 Re-analyze Item';
}
